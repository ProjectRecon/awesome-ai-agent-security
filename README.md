...
- **[Forge Agent Gate](https://github.com/forgeorbital/forge-agent-gate)** - Sits between an agent and its tools to gate irreversible actions (payments, refunds, deletes) behind a deterministic policy check before they execute, then writes a cryptographically signed, tamper-evident decision record. Open-source (MIT) TypeScript package and MCP server.
